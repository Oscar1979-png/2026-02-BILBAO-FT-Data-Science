...
os.chdir(os.path.dirname(__file__))     ## coge la ruta de tu script .py y pone la terminal en ese punto

data = pd.read_csv('data/Advertising.csv', index_col=0)     ## cargo data
...
pickle.dump(model, open('ad_model.pkl', 'wb'))              ## pickel con el modelo ya entrenado


app = flask.Flask(__name__)
app.config["DEBUG"] = True          ## se cambia a False cuando tenemos la version buena de produccion

@app.route('/', methods=['GET'])            ## decorador, landing page
def hello(): # Ligado al endopoint "/" o sea el home, con el método GET
    return "<h1>Bienvenido a mi API del modelo advertising</h1>"

@app.route('/api/v1/predict', methods=['GET'])      ## decorador para predicciones
def predict(): # Ligado al endpoint '/api/v1/predict', con el método GET
    ...
    if tv is None or radio is None or newspaper is None:
        return "Args empty, the data are not enough to predict"     ## si hay alguna vacia, aviso
    else:
...
def retrain(): # Rutarlo al endpoint '/api/v1/retrain/', metodo GET
    if os.path.exists("data/Advertising_new.csv"):
        ...
        return f"Model retrained. New evaluation metric RMSE: {str(rmse)}, MAPE: {str(mape)}"       ## mensaje con las metricas
    else:
        return f"New data for retrain NOT FOUND. Nothing done!"     ## mensaje alternativo si no hecho
