...
def get_by_id():
    id = int(request.get_json()['id'])      ## recibe un json con el id del libro
    for book in books:
        if book['id'] == id:
            return jsonify(book)
    return jsonify({'message': "Book not found"})

@app.route('/api/v1/resources/book', methods=['POST'])      ## para añadir libros
def post_book():
    data = request.get_json()
    books.append(data)
    return data

app.run()       ## mantiene funcionando todo el script
